...
export const search_tournament = async (user_input_tournament, player_name, game_name) =>  {
  ...
  const tournament_object_to_number = await Math.ceil(Number(get_tournament_object_to_number.total_count)/100);
  const result = load_bracket(tournament_object_to_number, player_name, user_input_tournament, game_name);
  return result;
}
